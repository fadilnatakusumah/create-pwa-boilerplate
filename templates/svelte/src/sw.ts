...
sw.addEventListener('push', (event: PushEvent) => {
  ...
  event.waitUntil(
    sw.registration.showNotification(data.title, { // registration property is accessed via sw
      body: data.body,
      icon: data.icon,
      badge: data.icon,
      data: data.data
...
sw.addEventListener('notificationclick', (event: NotificationEvent) => {
  // Error Fixes: 'notification' is a property of NotificationEvent
  event.notification.close();

  // 'clients' is a property of the ServiceWorkerGlobalScope
  event.waitUntil(
